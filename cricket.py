...
class BattingScore:
    def __init__(
        self, name, dismissal, runs, balls, sixes, fours, strike_rate, type, is_out
    ):
        self.name = name
...
class Innings:
    ...
    def print_bowling(self, out_file):
        ...
        for bowler in self.bowling_scorecard:
            bowling.add_row(
                [bowler.name, bowler.overs, bowler.runs, bowler.wickets, bowler.economy]
            )
...
if wait_for_start.total_seconds() > 0:
    ...
    if days > 0:
        print(
            f":{_home_team}: v :{_away_team}: {days}:{hours:02}:{mins:02} | symbolize=false"
        )
    else:
    ...
    print(
        f"{config['homeTeam']} vs {config['awayTeam']} starts on {date_time} | color = royalblue"
    )
    exit()
